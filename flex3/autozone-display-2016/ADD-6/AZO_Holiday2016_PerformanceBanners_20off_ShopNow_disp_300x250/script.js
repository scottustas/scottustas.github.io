...
startAnimation = function() {
  ...
  tl.from('#txt1', .65, { x: -245, ease: easeIn })
  tl.to('#txt1',   .65, { delay: 1.75, x: 300,  ease: easeOut })

  tl.from('#txt2', .65, { x: -300, ease: easeIn })
  tl.to('#txt2',   .65, { delay: 1.75, x: 300,  ease: easeOut })

  tl.add('cta');
  tl.from('#cta_txt', .65, { left: -275, ease: easeIn }, 'cta');
  tl.from('#cta', .65, { left: -275, ease: easeIn }, 'cta+=.1');
...
ctaRollAni = function(){
  if(ctaPlaying != true){
    ...
    TweenMax.to('#shine', 0.75, { opacity:0.7, x:145, ease:Power2.easeIn, onComplete: function() {
