...
function startAnimation(){
  ...
  TweenMax.to("#sol", 0, {delay: 0, scale:1.25, x:"+=5", transformOrigin:"right", ease: Linear.easeNone});
  TweenMax.to("#background", 14, { x:"+=350", transformOrigin:"right", ease: Linear.easeNone});
  // TweenMax.to("#sol", .75, {delay: 1, scale:.15, x:"+=15", y:"+=95", transformOrigin:"right", ease: Linear.easeNone});
  TweenMax.to("#sol", .75, {delay: 1, scale:.15, x:"+=40", y:"+=105", transformOrigin:"right", ease: Linear.easeNone});
  // TweenMax.to("#text", .25, {delay: 2, left:80, top:20, transformOrigin:"right", ease: Linear.easeNone, onComplete:addListeners});
   TweenMax.to("#text", .25, {delay: 2, scale:1.13, left: -12, top:1, transformOrigin:"right", ease: Linear.easeNone, onComplete:addListeners});
  //TweenMax.to("#cta", .25, {delay: 2, left:15, top:411, transformOrigin:"right", ease: Linear.easeNone,  onComplete:addListeners});
